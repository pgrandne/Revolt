    ...
    <div className="bg-slate-900 bg-opacity-90 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0 z-20">
      ...
      >
        ...
        >
          ...
          {loading ? t("processing") : t("nextbutton")}
        </button>
      </div>
    </div>
  );
...
export const ModalSelectChapterV1 = ({
  setModalSelectChapterV1,
  wallet,
}: {
  ...
  useEffect(() => {
    if (typeof address !== "undefined") setResumeButton(t("resume"));
  }, [address]);
  ...
  return (
    <div className="bg-slate-900 bg-opacity-60 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0 z-30">
      <div
        className={`flex flex-col bg-[#0f1216] px-2 sm:px-16 py-2 sm:py-14 gap-2 rounded-md text-center w-2/5`}
      >
        <p className="mb-4 text-xl">{t("title")}</p>
        <LinkLocale className="btnProgression" href="/chapter1/scene1">
          {t("chapter")} 1
        </LinkLocale>
        {wallet && isConnected && (
          <LinkLocale className="btnProgression" href="/chapter2/scene1">
            {t("chapter")} 2
          </LinkLocale>
        )}
        {(!wallet || !isConnected) && (
          <LinkLocale className="btnProgression" href="/chapter2">
            {t("chapter")} 2
          </LinkLocale>
        )}
        {wallet && (
    ...
    <div className="bg-slate-900 bg-opacity-90 flex justify-center items-center absolute top-0 right-0 bottom-0 left-0 z-30">
      <div className="font-permarker flex flex-col bg-[#0f1216] px-2 sm:px-16 py-2 sm:py-14 gap-2 rounded-md text-center w-2/5">
        <p className="mb-4 text-2xl">
          {" "}
          {t("text1")} <a className="underline"> {t("text2")} </a>{" "}
        </p>
        <p className="mb-4 text-sm"> {t("duration")} </p>
        <a
          className="bg-red-500 hover:bg-red-700 px-7 py-2 ml-2 rounded-md text-2xl text-white font-semibold"
          target="_blank"
          href="https://msprr0gajgn.typeform.com/to/DSl54TqJ#url=chap1scene7"
          onClick={() => {
